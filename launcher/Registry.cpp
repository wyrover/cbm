...
void CRegEntry::ForceStr()
{
    ...
    if ( lpszStr == NULL )
    {
        lpszStr = new _TCHAR[_MAX_REG_VALUE];
        lpszStr[0] = 0;
    }
}
...
CRegEntry& CRegEntry::operator=( CRegEntry& cregValue )
{
    ...
    if ( lpszName == NULL )
    {
        ...
        lpszName = new _TCHAR[nNameLen];
        _tcsncpy( lpszName, cregValue.lpszName, nNameLen );
    }
    ...
    case REG_BINARY:
    {
        size_t n = cregValue.vBytes.size();
        LPBYTE buf = new BYTE[n];
        cregValue.GetBinary( buf, n );
        SetBinary( buf, n );
        delete [] buf;
        return *this;
    }
...
void CRegEntry::SetBinary( LPBYTE lpbValue, size_t nLen )
{
    ...
    do
    {
        vBytes.push_back( lpbValue[vBytes.size()] );
    }
...
CRegEntry& CRegistry::operator []( LPCTSTR lpszVName )
{
    ...
    for ( int i = ( int )_reEntries.size() - 1; i >= 0; i-- )
    {
...
bool CRegistry::Refresh()
{
    ...
    for( DWORD dwIndex = 0; dwIndex < dwValueCount; dwIndex++ )
    {
        ...
        dwNameLen = sizeof( cValueName );
        dwBufferSize = _MAX_REG_VALUE;
        cValueName[0] = 0;
...
void CRegistry::DeleteKey( HKEY hPrimaryKey, LPCTSTR lpszSubKey )
{
    ...
    if ( !_tcslen( lpszSubKey ) || !hPrimaryKey )
    {
        assert( hPrimaryKey != NULL );
        goto cleanup;
    }
