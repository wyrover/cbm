...
public:
    ...
    CRegEntry&	operator =( DWORD dwValue )
    {
        return ( *this = &dwValue );
    }
    operator LPTSTR();
    ...
    bool		Convertible()
    {
        return __bConvertable;
    }

    __inline	void SetOwner( CRegistry* Owner )
    {
        __cregOwner = Owner;
    }

    template <class T>void SetStruct( T& type )
    {
        SetBinary( ( LPBYTE ) &type, sizeof( T ) );
    }
    template <class T>void GetStruct( T& type )
    {
        GetBinary( ( LPBYTE ) &type, sizeof( T ) );
    }

    __inline	bool IsString()
    {
        return ( iType == REG_SZ );
    }
    __inline	bool IsDWORD()
    {
        return ( iType == REG_DWORD );
    }
    __inline	bool IsBinary()
    {
        return ( iType == REG_BINARY );
    }
    __inline	bool IsMultiString()
    {
        return ( iType == REG_MULTI_SZ );
    }

    __inline	bool IsStored()
    {
        return __bStored;
    }
    __inline	bool Exists()
    {
        return __bStored;
    }

    __inline	void MultiClear()
    {
        SetMulti( _T( "/0" ), 2 );
    }
    __inline	void MultiAdd( LPCTSTR lpszVal )
    {
        MultiSetAt( MultiCount(), lpszVal );
    }
