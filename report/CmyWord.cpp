...
void CmyWord::UpdateField(CString strBMark)
{
	...
	m_wdSel.GoTo(COleVariant(short(-1)),COleVariant((short)0),COleVariant((short)0),varBMark_del);
...
BOOL CmyWord::Goto( const CString& strBMark )
{
	...
	m_wdSel.GoTo(COleVariant(short(-1)),COleVariant((short)0),COleVariant((short)0),varBMark);
	return TRUE;
}
...
*/
void CmyWord::SetTableText( int nRow, int nCell, const CString& text,float fontSize,long align )
{
...
BOOL CmyWord::GetBookMarks(CStringArray& bookMks)
{
	bookMks.RemoveAll();
	Bookmarks bMarks = m_wdDoc.GetBookmarks();
	long numBookMks = bMarks.GetCount();
	if(numBookMks < 1) return FALSE;
	for(int i = 1; i <= (int)numBookMks; i++)
	{
		VARIANT index;
		VariantInit(&index);
		index.vt = VT_INT;
		index.pdispVal = (IDispatch*)i;
		Bookmark bookMk = bMarks.Item(&index);
		bookMks.Add(bookMk.GetName());
	}
	return TRUE;
}
