...
//EvalUnit 类型的CRUD操作
int32_t SQLClientHelper::AddEvalUnit(const cbm::EvalUnit & eval_unit) {
	int32_t ret = -1;
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
...
bool SQLClientHelper::DeleteEvalUnit(const int32_t id) {
	bool ret = false;
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
...
bool SQLClientHelper::UpdateEvalUnit(const cbm::EvalUnit & eval_unit) {
	bool ret = false;
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
...
void SQLClientHelper::GetEvalUnitById(cbm::EvalUnit & _return, const int32_t id) {
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
		service_client.get()->GetEvalUnitById(_return, id);
		service_client.close();
	}
	catch (TException &tx) {
		std::string error_msg = tx.what();
...
void SQLClientHelper::GetEvalUnitByForeignKey(cbm::EvalUnit & _return, const std::string& fkey, const int32_t id) {
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
		service_client.get()->GetEvalUnitByForeignKey(_return, fkey, id);
		service_client.close();
	}
	catch (TException &tx) {
		std::string error_msg = tx.what();
...
int32_t SQLClientHelper::GetEvalUnitIdByForeignKey(const std::string& fkey, const int32_t id) {
	int32_t ret = -1;
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
		ret = service_client.get()->GetEvalUnitIdByForeignKey(fkey, id);
		service_client.close();
	}
	catch (TException &tx) {
		std::string error_msg = tx.what();
...
void SQLClientHelper::GetEvalUnitList(std::vector<cbm::EvalUnit> & _return) {
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
		service_client.get()->GetEvalUnitList(_return);
		service_client.close();
	}
	catch (TException &tx) {
		std::string error_msg = tx.what();
...
void SQLClientHelper::GetEvalUnitIds(std::vector<int32_t> & _return) {
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
		service_client.get()->GetEvalUnitIds(_return);
		service_client.close();
	}
	catch (TException &tx) {
		std::string error_msg = tx.what();
...
void SQLClientHelper::GetEvalUnitNames(std::vector<std::string> & _return) {
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
		service_client.get()->GetEvalUnitNames(_return);
		service_client.close();
	}
	catch (TException &tx) {
		std::string error_msg = tx.what();
...
void SQLClientHelper::AddMoreEvalUnit(const std::vector<cbm::EvalUnit> & objs) {
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
...
void SQLClientHelper::GetEvalUnitByFields(cbm::EvalUnit & _return, const std::map<std::string, std::string> & fields) {
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
		service_client.get()->GetEvalUnitByFields(_return, fields);
		service_client.close();
	}
	catch (TException &tx) {
		std::string error_msg = tx.what();
...
void SQLClientHelper::GetEvalUnitByField1(cbm::EvalUnit & _return, const std::string& field, const std::string& value) {
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
		service_client.get()->GetEvalUnitByField1(_return, field, value);
		service_client.close();
	}
	catch (TException &tx) {
		std::string error_msg = tx.what();
...
void SQLClientHelper::GetEvalUnitByField2(cbm::EvalUnit & _return, const std::string& field1, const std::string& value1, const std::string& field2, const std::string& value2) {
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
...
void SQLClientHelper::GetEvalUnitListByFields(std::vector<cbm::EvalUnit> & _return, const std::map<std::string, std::string> & fields) {
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
		service_client.get()->GetEvalUnitListByFields(_return, fields);
		service_client.close();
	}
	catch (TException &tx) {
		std::string error_msg = tx.what();
...
void SQLClientHelper::GetEvalUnitListByField1(std::vector<cbm::EvalUnit> & _return, const std::string& field, const std::string& value) {
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
		service_client.get()->GetEvalUnitListByField1(_return, field, value);
		service_client.close();
	}
	catch (TException &tx) {
		std::string error_msg = tx.what();
...
void SQLClientHelper::GetEvalUnitListByField2(std::vector<cbm::EvalUnit> & _return, const std::string& field1, const std::string& value1, const std::string& field2, const std::string& value2) {
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
		service_client.get()->GetEvalUnitListByField2(_return, field1, value1, field2, value2);
		service_client.close();
	}
	catch (TException &tx) {
		std::string error_msg = tx.what();
...
int32_t SQLClientHelper::GetEvalUnitIdByFields(const std::map<std::string, std::string> & fields) {
	int32_t ret = -1;
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
		ret = service_client.get()->GetEvalUnitIdByFields(fields);
		service_client.close();
	}
	catch (TException &tx) {
		std::string error_msg = tx.what();
...
int32_t SQLClientHelper::GetEvalUnitIdByField1(const std::string& field, const std::string& value) {
	int32_t ret = -1;
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
		ret = service_client.get()->GetEvalUnitIdByField1(field, value);
		service_client.close();
	}
	catch (TException &tx) {
		std::string error_msg = tx.what();
...
int32_t SQLClientHelper::GetEvalUnitIdByField2(const std::string& field1, const std::string& value1, const std::string& field2, const std::string& value2) {
	int32_t ret = -1;
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
...
void SQLClientHelper::GetEvalUnitIdListByFields(std::vector<int32_t> & _return, const std::map<std::string, std::string> & fields) {
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
		service_client.get()->GetEvalUnitIdListByFields(_return, fields);
		service_client.close();
	}
	catch (TException &tx) {
		std::string error_msg = tx.what();
...
void SQLClientHelper::GetEvalUnitIdListByField1(std::vector<int32_t> & _return, const std::string& field, const std::string& value) {
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
		service_client.get()->GetEvalUnitIdListByField1(_return, field, value);
		service_client.close();
	}
	catch (TException &tx) {
		std::string error_msg = tx.what();
...
void SQLClientHelper::GetEvalUnitIdListByField2(std::vector<int32_t> & _return, const std::string& field1, const std::string& value1, const std::string& field2, const std::string& value2) {
	try {
		RpcClient<cbm::CbmServiceClient> service_client(HOST, PORT2);
		service_client.start();
		service_client.get()->GetEvalUnitIdListByField2(_return, field1, value1, field2, value2);
		service_client.close();
	}
	catch (TException &tx) {
		std::string error_msg = tx.what();
