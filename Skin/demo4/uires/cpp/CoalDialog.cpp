...
LRESULT CoalDialog::OnInitDialog( HWND hWnd, LPARAM lParam )
{
	...
	m_StabilityEdit = FindChildByName2<SEdit>(L"stability");
	...
	m_MineIndexCombox = FindChildByName2<SComboBox>(L"mine_index");
...
void CoalDialog::OnMineIndexComboxSelChanged(SOUI::EventArgs *pEvt)
{
	if(!m_bLayoutInited) return;
	EventCBSelChange* pEvtOfCB = (EventCBSelChange*)pEvt;
	if(pEvtOfCB == 0) return;
