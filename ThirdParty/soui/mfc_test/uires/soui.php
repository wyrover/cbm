  ...
  参考示例:(这里为了便于描述,格式化成树状结构,但实际应用中都是格式化成一行后再进行传递)
{
  ...
  */
    ...
    //(3)最大最小关闭按钮
    $btn_names = array('min'=>1, 'max'=>1, 'close'=>1);
...
function createInitXmlFile($uiResFile='uires.idx', $resType='img')
{
  ...
  foreach ($doc->getElementsByTagName($resType) as $nodes) {
   foreach ($nodes->childNodes as $node) {
     ...
     $sql = "INSERT INTO skin(name, src) VALUES ('$name', '$path')";
     $mysqli->query($sql);
   }
 }
 /* close connection */
...
mockups_to_soui_converter();
//生成资源入口文件uires.idx
createUIResFile('uires.idx');
//生成图片资料列表
createInitXmlFile('uires.idx', 'img');
